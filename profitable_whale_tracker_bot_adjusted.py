#!/usr/bin/env python3
"""
Whale 17 Position Tracker V2 - RAILWAY VERSION (FIXED)
- Double-checks position closes before sending notifications
- Bold formatting for token/direction
- Removed embedded links from partial notifications
...
class WhaleTracker:
    """
    Whale Position Tracker V2 - Railway Version (FIXED)
    """
    def __init__(self):
        ...
        print("WHALE POSITION TRACKER V2 - RAILWAY VERSION (FIXED)")
    ...
    def send_open_alert(self, coin, side, value, price):
        """Send OPEN position alert with bold token/direction"""
...
📝 <a href="{coinglass_url}">{self.whale_name}</a>
...
₿ BTC: ${self.btc_price:,.0f}"""
    ...
    def send_close_alert(self, coin, side, value, price, entry_price=None):
        """Send CLOSE position alert with bold token/direction"""
        emoji, tier = self.get_position_emoji_and_tier(coin, value)
        coinglass_url = f"https://www.coinglass.com/hyperliquid/{self.whale_address}"
...
₿ BTC: ${self.btc_price:,.0f}"""
    ...
    def send_partial_close_alert(self, coin, side, prev_value, curr_value, reduction_pct, price):
        """Send PARTIAL CLOSE alert with bold token/direction, NO embedded link"""
...
₿ BTC: ${self.btc_price:,.0f}"""
    ...
    def send_partial_increase_alert(self, coin, side, prev_value, curr_value, increase_pct, price):
        """Send PARTIAL INCREASE alert with bold token/direction, NO embedded link"""
...
₿ BTC: ${self.btc_price:,.0f}"""
    ...
    async def start(self):
        print(f"\nSTARTING WHALE POSITION TRACKER V2 (FIXED)")
        print("=" * 50)
...
def main():
    print("WHALE POSITION TRACKER V2 - RAILWAY (FIXED)")
    print("=" * 50)
    print("Features:")
    ...
    print("• Bold formatting for tokens/directions")
